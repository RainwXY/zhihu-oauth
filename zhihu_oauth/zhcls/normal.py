...
def normal_attr(name_in_json=None):
    ...
    1. 如果 ``data`` 存在，尝试从 ``data`` 中取属性，
       找不到的话则返回被装饰函数的执行结果。
    2. 如果上一步取属性失败，则尝试从 ``cache`` 中取需要的属性。
    3. 如果上一步取属性失败（没有 ``cache`` 或 ``cache`` 中不存在需要的属性，
       或其类型不是基本类型）则判断属性名是不是 ``id``。
    4. 是 ``id`` 的话则返回被装饰函数的执行结果。（因为 ``id`` 属性一般在
       :any:`_build_url` 方法中需要引用，如果这时向知乎请求数据会造成死循环。
    5. 不是 ``id`` 的话如则使用 API 请求数据。然后执行步骤 1。
